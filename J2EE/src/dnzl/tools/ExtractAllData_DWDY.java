 ...
 * @summary  用于手动抽取数据
 *****************************************************************************/
public class ExtractAllData_DWDY {
	
	private static String  start_time="2014-02-26 19:00:00";
	
	private static String  end_time = "2014-02-27 03:00:00";
...
public List<SupperVo> initVo_MXDY() throws Exception{
		...
		String sql = "SELECT  ID, CODE MX_CODE, " +
				...
				"and to_date(to_char(ddate,'yyyy-MM-dd')||' '||HOUR||':00:00') >= DATEADD(HH,-1,to_date('"+start_time+"')) "+
			    "and to_date(to_char(ddate,'yyyy-MM-dd')||' '||HOUR||':00:00') <= DATEADD(HH,-1,to_date('"+end_time+"')) ";
	...
	public static void main(String[] args) throws Exception{
		ExtractAllData_DWDY obj = new ExtractAllData_DWDY();
		obj.initdb();
		obj.save(obj.initVo_MXDY());
	}
