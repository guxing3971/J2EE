...
public class PushAllData_DWDY {
	...
	public static void main(String[] args) {
		...
		String sql = "SELECT recordid,companyid, companyName, MX_CODE,"
				...
				+ " and mx_code in (select mx_code from dnzlZHILIANG.HANDLERESULT_DEV_MX"
				+ " where status='no' and VOLTAGEGRADE >110)";
		...
		try {
			XmlFactory.DWDY =sql;
			xml_list = XmlFactory.GetXMLAll_DWDY();
		...
		try {
			while (iter.hasNext()) {
				...
				if(!(F_Count == 0)){
					...
					httpClientPolicy.setReceiveTimeout(60 * 1000 * 10);
